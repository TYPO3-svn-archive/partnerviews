...
/**
 *
 * This class is the controller creating new records of the partner-extension.
 *
 * PHP version 5
 *
 ...
 *
 */
class tx_partnerviews_controllers_fenew extends tx_lib_controller {
	...
	function selectAction(){
		...
		$actionPipe = $this->makeInstance('tx_partnerviews_views_fenewform', $actionPipe);
		$fe_userid = (integer) $GLOBALS['TSFE']->fe_user->user['uid'];
		if ($fe_userid <> "0"){ 
			$actionPipe->render($this->configurations['feNewSelectTemplate'], TRUE);
		} else {
	...
	function storeAction(){
		...
		$actionPipe->render($this->configurations['feNewStoreTemplate'], TRUE);
		$actionPipe = $this->makeInstance('tx_lib_translator', $actionPipe);
		$actionPipe->translate();
		$actionPipe = $this->makeInstance('tx_lib_sessionProcessor', $actionPipe);
		$actionPipe->storeSession();
